import re
import matplotlib
matplotlib.use("TkAgg")  # or "Qt5Agg", "Agg", etc. depending on what's available
import matplotlib.pyplot as plt


# Paste your full log text here as a triple-quoted string
log_text = """
...
# Extract timesteps (based on evaluation steps)
timesteps = [int(m.group(1)) for m in re.finditer(r"Eval num_timesteps=(\d+)", log_text)]

# Extract policy_gradient_loss
pg_losses = [float(m.group(1)) for m in re.finditer(r"policy_gradient_loss\s+\|\s+([-\d\.eE+]+)", log_text)]

# Check if lengths match
if len(timesteps) != len(pg_losses):
    print(f"⚠️ Warning: Mismatch — {len(timesteps)} timesteps vs {len(pg_losses)} policy_gradient_loss entries")

# Plot
plt.figure(figsize=(8, 4))
plt.plot(timesteps[:len(pg_losses)], pg_losses, marker='o', linestyle='-', color='green')
plt.xlabel("Timesteps")
plt.ylabel("Policy Gradient Loss")
plt.title("Policy Gradient Loss over Timesteps")
plt.grid(True)
plt.tight_layout()
plt.show()